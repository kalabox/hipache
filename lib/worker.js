...
Worker.prototype.runServer = function (config) {
    ...
    var startHandler = function (req, res) {
        ...
        if (req.headers['x-forwarded-protocol'] === undefined) {
            req.headers['x-forwarded-protocol'] = req.connection.encrypted ? 'https' : 'http';
        }
        ...
        if (req.headers['x-forwarded-port'] === undefined) {
            ...
            req.headers['x-forwarded-port'] = req.connection.encrypted ? '443' : '80';
        }
    };
    ...
    var httpRequestHandler = function (req, res) {
        ...
        var proxyRequest = function () {
            this.cache.getBackendFromHostHeader(req.headers.host, req.connection.encrypted, function (err, code, backend) {
                if (err) {
